...
class SimpleTrainer(TrainerBase):
    ...
    def build_data_loader(self):
        ...
        self.retrieval_loader = dm.retrieval_loader  # optional, can be None
...
class TrainerXU(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        len_retrieval_loader = len(self.retrieval_loader)
        if self.cfg.TRAIN.COUNT_ITER == "train_x":
            self.num_batches = len_train_loader_x
        elif self.cfg.TRAIN.COUNT_ITER == "retrieval":
            self.num_batches = len_retrieval_loader
        elif self.cfg.TRAIN.COUNT_ITER == "smaller_one":
            self.num_batches = min(len_train_loader_x, len_retrieval_loader)
        else:
            raise ValueError

        train_loader_x_iter = iter(self.train_loader_x)
        retrieval_loader_iter = iter(self.retrieval_loader)
        ...
        for self.batch_idx in range(self.num_batches):
            ...
            try:
                batch_u = next(retrieval_loader_iter)
            except StopIteration:
                retrieval_loader_iter = iter(self.retrieval_loader)
                batch_u = next(retrieval_loader_iter)
