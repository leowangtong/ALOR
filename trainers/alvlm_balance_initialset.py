...
class TextEncoder(nn.Module):
    def __init__(self, clip_model, cfg):
        ...
        if 'open' not in cfg.MODEL.BACKBONE.NAME:
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model, dataset_name):
        super().__init__()
        ...
        if dataset_name == 'semi_aves':
            if cfg.TRAINER.COOPAL.ASPATH:
                ...
                for name in classnames:
                    name = name.lower()
                    for desc in desc_dict[name]['class_name_prompts']:
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, dataset_name, desc_file=None):
        super().__init__()

        self.prompt_learner = PromptLearner(cfg, classnames, clip_model, dataset_name)
    ...
    def clf(self, emb):
        ...
        logit_scale = self.logit_scale.exp()
        logits = logit_scale * emb.half() @ text_features.t()
...
class ALVLM_balance_initialset(TrainerX):
    ...
    def build_model(self):
        ...
        if 'open' not in cfg.MODEL.BACKBONE.NAME:
            ...
            arch = cfg.MODEL.BACKBONE.NAME.split('_')[0]
            corpus_config, model_arch = OPENCLIP_MODEL_DIC['laion400m'][arch]
            clip_model = open_clip.create_model_and_transforms(model_arch, pretrained=corpus_config)
            clip_model = clip_model[0].cuda().eval()
            state_dict = None
            clip_model = clip.build_model(state_dict or clip_model.state_dict())
            self.clip_model = clip_model

            #clip_model = clip_model[0].cuda().half()
        ...
        if cfg.TRAINER.COOPAL.ASPATH:
            self.model = CustomCLIP(cfg, classnames, clip_model, dataset_name, desc_file=cfg.TRAINER.COOPAL.ASPATH)
        elif cfg.TRAINER.COOPAL.AEPATH:
            self.model = CustomCLIP(cfg, classnames, clip_model, dataset_name, desc_file=cfg.TRAINER.COOPAL.AEPATH)
        else:
            self.model = CustomCLIP(cfg, classnames, clip_model, dataset_name)
        print(self.model)
    ...
    def train(self):
        ...
        dataset._train_x = dataset.retrieval
        for i in range(7):
            start = time.time()
            if i == 0:
                idx = []
                train_dict = {}
                num_of_images_perclass = 1

                for a in range(n_query):
                    train_dict[str(a)] = []
                    for j in range(len(unlabeled_dst)):
                        if unlabeled_dst[j].label == a:
                            train_dict[str(a)].append(j)

                for a in range(n_query):
                    random.seed(self.cfg.SEED)
                    initials = sample(train_dict[str(a)], num_of_images_perclass)
            ...
            if i != 0:
                ...
                elif self.cfg.TRAINER.COOPAL.METHOD == "TFS":
                    selector = TFS(self.cfg, self.model, unlabeled_dst, U_index, dataset.get_num_classes(unlabeled_dst), dataset, self.device)
                    idx = selector.select(n_query)
                elif self.cfg.TRAINER.COOPAL.METHOD == "entropy":
                    selector = Entropy(self.cfg, self.model, unlabeled_dst, U_index, dataset.get_num_classes(unlabeled_dst),
                                       self.device)
                    idx = selector.select(n_query)
                elif self.cfg.TRAINER.COOPAL.METHOD == "logo":
                    selector = LoGo(self.cfg, self.model, unlabeled_dst, U_index, dataset.get_num_classes(unlabeled_dst), self.device)
                    idx = selector.query(n_query)
                elif self.cfg.TRAINER.COOPAL.METHOD == "alfa_mix":
                    val_x = dataset._train_x.copy()
                    selector = ALFAMix(self.cfg, self.model, unlabeled_dst, U_index, val_x, dataset.get_num_classes(unlabeled_dst), self.device)
                    idx = selector.query(n_query)
                elif self.cfg.TRAINER.COOPAL.METHOD == "badge":
                    selector = BADGE(self.cfg, self.model, unlabeled_dst, U_index, dataset.get_num_classes(unlabeled_dst),
                                     self.device)
                    idx = selector.select(n_query)
                elif self.cfg.TRAINER.COOPAL.METHOD == "badge_pcb":
                    selector = BADGE(self.cfg, self.model, unlabeled_dst, U_index, dataset.get_num_classes(unlabeled_dst),
                                     self.device)
                    idx = selector.select(n_cand)
                    statistics = torch.zeros(dataset.get_num_classes(unlabeled_dst))
                    for elem in dataset._train_x:
                        statistics[elem.label] += 1
                    selector = PCB(self.cfg, self.model, unlabeled_dst, idx, dataset.get_num_classes(unlabeled_dst),
                                statistics, self.device)
                    idx = selector.select(n_query)
            ...
            for k in idx:
                ...
                U_index.remove(k)
                labels.append(unlabeled_dst[k].label)
