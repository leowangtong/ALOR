...
class ALFAMix(AL):
    ...
    def get_embedding(self, dataloader, data):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                out, features = self.model(inputs, get_feature=True)
                for j in range(len(inputs)):
                    embedding[i * self.cfg.DATALOADER.TRAIN_X.BATCH_SIZE + j] = features[j].data.cpu()

        return embedding

    def query(self, n_query):
        unlabel_idxs = np.array(self.U_index)
        label_idxs = np.array(range(len(self.labeled_in_set)))
        ...
        org_ulb_embedding = self.get_embedding(unlabeled_loader, unlabel_idxs)
        ...
        org_lb_embedding = self.get_embedding(labeled_in_loader, label_idxs)
    ...
    def get_labels(self, data_idxs):
        with torch.no_grad():
            ...
            for i, data in enumerate(labeled_in_loader):
                if i == 0:
                    labels = copy.deepcopy(data['label'])
                else:
                    labels = torch.cat([labels, copy.deepcopy(data['label'])], dim=0)
    ...
    def find_candidate_set(self, lb_embedding, ulb_embedding, pred_1, ulb_probs, alpha_cap, Y, grads):
        ...
        for i in range(self.n_class):
            ...
            if self.alpha_closed_form_approx:
                ...
                out = self.model.clf(embedding_mix)
                out = out.detach().cpu()
                alpha = alpha.cpu()
